...
> **v3-уточнение (2026-09-20).** Кроме C1/C1b этим же прогоном сняты или снижены: **V3** («5 потерянных ресурсов» — не дефект; и «дыра GIF» из v2 §3 тоже снята), **O7** (1 карта → 10 карт, 50 хитов вместо 25), **O8** (усилен: гасит пусковую навсегда, 19 объектов на 18 картах), **V2** (носителей «номера шага» 4, включая титул `STEP 10`), а также отозваны T1/M1/O10 и вывод о read-only `stageExitMarkers`. Авторитет: `exolon-full-audit-20260920-v3.md`.
